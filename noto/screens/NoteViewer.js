...
const NoteViewer = ({ route, navigation }) => {
  ...
  const handleStarred = () => {
    ...
    saveToStorage(data);
    if (!isStarred) ToastAndroid.show("You Starred a note", ToastAndroid.SHORT);
  };
  ...
  const saveToStorage = async (_storage) => {
    ...
    await AsyncStorage.setItem("@notes", _storageJson);
  };
  const getData = async () => {
  ...
  const getCurrent = (id, storage) => {
    let _current_note = storage.filter((item) => item.id == id);
    let _current = _current_note[0];
    ...
    <View style={styles.container}>
      <View style={styles.InputContainer}>
        ...
        <View style={styles.headerContainer}>
          ...
          <TouchableOpacity style={styles.headerIcons} onPress={handlePinClick}>
          ...
          <TouchableOpacity style={styles.headerIcons} onPress={handleStarred}>
            <AntDesign
