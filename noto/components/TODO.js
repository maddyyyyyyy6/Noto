...
export default function TODO({ id,title, text, state,navigation }) {
  const track = state;
  ...
  const handlePress = () => {
    navigation.navigate("TODOconfig",{id:id})
  };
  return (
    <TouchableOpacity
      style={[
        styles.doerContainer,
        { backgroundColor: getTrackColor(), borderColor: getNewBarTextColor() },
      ]}
      activeOpacity={0.7}
      onPress={handlePress}
    >
      <View style={styles.headerContainer}>
        <Text style={[styles.doerTitle, { color: getNewBarTextColor() }]}>
          {title}
        </Text>
      </View>
      <Text style={[styles.doerDes, { color: getNewBarTextColor() }]}>
        {text}
      </Text>
    </TouchableOpacity>
  );
